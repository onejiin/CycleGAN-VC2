...
class CycleGAN(object):
    ...
    def __init__(self, num_features, mode = 'train',
        ...
        # Model select
        # ----------------- CycleGAN-VC1 ----------------- #
        if gen_model == 'CycleGAN-VC1':
            self.generator = generator_gatedcnn
            self.discriminator = discriminator
        # ------------------ CyleGAN-VC2 ------------------ #
        elif gen_model == 'CycleGAN-VC2':
            self.generator = generator_gated2Dcnn
            self.discriminator = discriminator_2D
        elif gen_model == 'CycleGAN2_withDeconv':
            self.generator = generator_gated2Dcnn_withDeconv
            self.discriminator = discriminator_2D
        else:
            assert gen_model is None
